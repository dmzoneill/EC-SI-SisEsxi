...
sub acevm_included_player_newer {
  return 254719 gt int($gConfig->get('product.buildNumber', ''));
}
...
sub install_content_vicli {
  ...
  print "Installing version 254719 of " . vmware_product_name() . "\n\n\n";
  ...
  db_add_answer('BUILD_NUMBER', "254719");
...
sub main {
  ...
  if (internal_basename($0) eq $cModuleUpdaterFileName) {
    ...
    if (not (($installed_version eq '4.1.0') and
